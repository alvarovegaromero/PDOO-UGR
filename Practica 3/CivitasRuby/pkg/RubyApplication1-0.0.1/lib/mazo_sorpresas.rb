...
class MazoSorpresas
  ...
  def inhabilitar_carta_especial(sorpresa)
    if @sorpresas.include?(sorpresa)
      ...
      indice = @sorpresas.index(sorpresa) # No se hace @sorpresas.delete(sorpresa) 
      @sorpresas.delete_at(indice)        # por si hay dos cartas iguales que no borre las dos
      
      @cartas_especiales << sorpresa
      
      Diario.instance.ocurre_evento("Inhabilitamos carta " + sorpresa.nombre)
    end
  end
