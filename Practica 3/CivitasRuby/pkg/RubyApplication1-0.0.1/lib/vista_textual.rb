...
=end
    def comprar
      opcion = self.menu("¿Quieres comprar la calle a la que has llegado?",
                        ["SI","NO"] ) 
...
=end
    def gestionar
      opcion_gest = self.menu("¿Qué número de gestión inmobiliara quieres realizar?",
                                ["-> VENDER", 
                                 "-> HIPOTECAR",
                                 "-> CANCELAR_HIPOTECA", 
                                 "-> CONSTRUIR_CASA", 
...
=end
    def actualizar_vista
      ...
      if (@juegoModel.get_jugador_actual.propiedades.empty?)
        ...
        for i in 0..@juegoModel.get_jugador_actual.propiedades.length-1
      ...
      self.mostrar_estado(
                           @juegoModel.get_jugador_actual.nombre + "\n"+
                           "Sus propiedades son:\n"  +  propiedades + "\n"+
