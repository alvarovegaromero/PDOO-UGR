...
// @return Sorpresa activa en en ese momento para el juego
    Sorpresa siguiente(){
        ...
        System.out.println("Sorpritas: " + sorpresas.size());

        if((!barajada || usadas == sorpresas.size()) && !debug){
            System.out.println("Se esta barajando\n\n\n");
            Collections.shuffle(sorpresas); //Barajamos con la función shuffle
            usadas = 0;
            barajada = true;
        }
        
        // Si se usan todas las sorpresas la siguiente vez q saquemos peta
        usadas++;
        ultimaSorpresa = sorpresas.get(0); //Primer elemento del vector
        sorpresas.remove(0);    
        
        //Añadimos al final para no perderla
