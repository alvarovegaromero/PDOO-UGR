...
def self.new_copia(jugador)
  self.new(jugador.nombre,jugador.encarcelado, jugador.num_casilla_actual,
  ...
  if(!encarcelado)
    ...
    if(existe)
      ...
      if(puedo_edificar_casa)
        ...
        if(result)
          Diario.instance.ocurre_evento("El jugador #{@nombre} construye una casa en #{ip} (#{propiedad.num_casas})")
        end
      end
    end
  end
