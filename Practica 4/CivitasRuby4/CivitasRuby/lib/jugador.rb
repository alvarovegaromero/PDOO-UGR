...
attr_reader :nombre, :saldo, :saldo_inicial, :casas_max, :hoteles_max, :casas_por_hotel,:num_casilla_actual,
            :precio_libertad,:paso_por_salida,:propiedades,:puede_comprar,:encarcelado, :salvoconducto
...
def self.new_nombre(nombre)
  self.new(nombre, false, 0, false, @@saldo_inicial, nil, Array.new)
end
...
def copia(jugador)
  ...
  @salvoconducto      = jugador.salvoconducto #Objeto de Sorpresa
end
  ...
  if(!encarcelado)
    ...
    if(existe)
      ...
      if(puedo_edificar_casa)
        ...
        if(result)
          Diario.instance.ocurre_evento("El jugador #{@nombre} construye una casa en #{propiedad.to_s} asi que tiene #{propiedad.num_casas}")
        else 
          Diario.instance.ocurre_evento("El jugador #{@nombre} no construye casa en #{propiedad.to_s}. Tiene #{propiedad.num_casas}")
        end
...
def construir_hotel(ip)
  ...
  if(!@encarcelado && existe_la_propiedad(ip))
    ...
    if(puedo_edificar_hotel)
      ...
      Diario.instance.ocurre_evento("El jugador #{@nombre} construye un hotel en #{@propiedades[ip].to_s}")
    end
  end
...
def encarcelar(num_casilla_carcel)
  if self.debe_ser_encarcelado
    if self.mover_a_casilla(num_casilla_carcel)
...
def mover_a_casilla(numCasilla)
  ...
  if !@encarcelado
    ...
    @puede_comprar = false
    Diario.instance.ocurre_evento("La nueva posicion de #{@nombre} es #{numCasilla}")
    resultado = true
  end
...
def salir_carcel_pagando 
  ...
  if (@encarcelado && puede_salir_carcel_pagando )
    ...
    Diario.instance.ocurre_evento("#{@nombre} no ha podido salir de la carcel pagando
...
def salir_carcel_tirando 
  ...
  if (Dado.instance.salgo_de_la_carcel)
    ...
    Diario.instance.ocurre_evento("#{@nombre} no ha podido salir de la carcel tirando
                          \nNo ha salido el numero requerido en los dados")
