...
=end
    def gestionar
      opcion_gest = self.menu("Que numero de gestion inmobiliara quieres realizar?",
                                ["-> VENDER", 
                                 "-> HIPOTECAR",
                                 "-> CANCELAR_HIPOTECA", 
                                 "-> CONSTRUIR_CASA", 
      ...
      if ( opcion_gest != 5 )
        ...
        for i in 0...num_props
          props[i] = "->" + @juegoModel.get_jugador_actual.propiedades[i].nombre
        end
        
        opcion_prop = self.menu("Dime sobre que propiedad quieres hacer la gestion: ",
                                props )
